...
class TransformerEncoder(nn.Module):
    def __init__(self, configs: dict):
        ...
        self.last_week = configs['last_week']
        self.last_day = configs['last_day']
    ...
    def _derive_graphs(self, x_batch, x_time_idx):
        ...
        for idx, x_all_t in enumerate(x_batch):
            ...
            if self.last_week and self.last_day:
                last_day_end = self.basic_input_len * 2
            if not self.last_week and self.last_day:
            ...
            for i, x in enumerate(x_all_t):
                ...
                if self.graph_semantic_input:
                    if last_day_end != -1 and i < last_day_end:
                        graph_semantic = self._create_graph(x, semantic_edge_index_lst_dy, semantic_edge_attr_lst_dy)
                    else:
                        graph_semantic = self._create_graph(x, semantic_edge_index_hr, semantic_edge_attr_hr)
    ...
    def forward(self, x, x_time_idx, enc_idx):
        ...
        if enc_idx == 0:
            ...
            out_graph, out_graph_semantic = self._derive_graphs(out, x_time_idx)
