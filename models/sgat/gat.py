...
class GAT(nn.Module):
    def __init__(self, configs):
        ...
        for l in range(self.n_layers):
            ...
            gat_layer = GATLayer(in_f_size, in_f_size, n_heads=n_heads[l], dropout=self.dropout_g, alpha=alpha,
                                 concat=concat, edge_dim=edge_dim, seq_len=seq_len)
            self.layer_stack.append(gat_layer)

        self.lin_out = nn.Linear(int(in_f_size/self.seq_len), self.out_f_sizes[-1])
    ...
    def forward(self, batch_data):
        ...
        for data in batch_data:
            ...
            for l, gat_layer in enumerate(self.layer_stack):
                ...
                x = x.reshape(x_shp[0], self.seq_len, -1)  # 307, 36, 288
        ...
        return self.lin_out(torch.stack(out))
