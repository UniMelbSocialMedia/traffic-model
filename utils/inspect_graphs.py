...
if __name__ == '__main__':
    ...
    for batch in range(0, 10):
        ...
        test_x = z_inverse(test_x, mean=data_loader.dataset.stats_train['mean'], std=data_loader.dataset.stats_train['std'])
        test_x = test_x.reshape((shape[0], 12, shape[2], 1))

        shape = test_y.shape
        test_y_target = test_y_target.transpose(1, 2)[:, :, 1:, 0:1].detach().cpu().numpy().reshape(
            (shape[0] * 12 * (shape[2]), 1))
        test_y_target = z_inverse(test_y_target, mean=data_loader.dataset.stats_train['mean'], std=data_loader.dataset.stats_train['std'])
